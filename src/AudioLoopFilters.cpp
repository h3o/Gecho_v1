...
void audio_loop_filters()
{
	...
	//int TEMPO_BY_SAMPLE = 2*I2S_AUDIOFREQ-AUDIOFREQ_DIV_CORRECTION; //120BPM (16s / loop)
	...
	//calculate from BPM
	//int TEMPO_BY_SAMPLE = get_tempo_by_BPM(120);
	//int TEMPO_BY_SAMPLE = get_tempo_by_BPM(160);
	//int TEMPO_BY_SAMPLE = get_tempo_by_BPM(200);
	...
	//send silence to codec
	for(int i=0;i<CODEC_WARM_UP_SAMPLES;i++)
	{
	...
	/*
	//phaser test
	#define PHASER_BUF_SIZE 256
	float phaser_buf[PHASER_BUF_SIZE];
	int phaser_buf_ptr = 0;
	int phaser_delay = PHASER_BUF_SIZE;
	int phaser_step = -2;
	int phaser_change = 0;
	#define PHASER_TIMING 1000
	*/
	...
	while(run_program)
	{
		...
		else //stereo sample - right channel
		{
			...
			if(PROG_add_OpAmp_ADC12_signal)
			{
				...
				*/
				...
				//by ref version:
				//sample_f[0] = (float)(4096 - (int16_t)ADC1_read()) / 4096.0f * 12;
				//sample_f[1] = (float)(4096 - (int16_t)ADC2_read()) / 4096.0f * 12;
				//sample_f[0] = (float)(4096 - (int16_t)ADC1->DR) / 4096.0f * 12;
				//sample_f[1] = (float)(4096 - (int16_t)ADC2->DR) / 4096.0f * 12;
				sample_f[0] = (float)(4096 - (int16_t)ADC1->DR) * OpAmp_ADC12_signal_conversion_factor;
				sample_f[1] = (float)(4096 - (int16_t)ADC2->DR) * OpAmp_ADC12_signal_conversion_factor;
		...
		*/
		...
		/*
		if(sample_mix > computed_sample_dynamic_limit)
		{
			sample_mix = computed_sample_dynamic_limit;
		}
		...
		*/
		...
		/*
		//------------------------------------------------------
		phaser_buf[phaser_buf_ptr] = sample_mix;
		phaser_buf_ptr++;
		if(phaser_buf_ptr>=phaser_delay)
		{
			phaser_buf_ptr = 0;
		}
		sample_mix += phaser_buf[phaser_buf_ptr];

		phaser_change++;
		if(phaser_change==PHASER_TIMING)
		{
			phaser_change = 0;
			phaser_delay += phaser_step;
			if(phaser_delay==0 || phaser_delay==PHASER_BUF_SIZE)
			{
				phaser_step = -phaser_step;
				if(phaser_step>0)
				{
					LED_SIG_ON;
				}
		...
		*/
		...
		if(PROG_enable_filters)
		{
			if(PROG_enable_chord_loop)
			{
				...
				//need to time along the tempoCounter, at every quarter
				{
				...
				*/
				...
				if((seconds % SHIFT_CHORD_INTERVAL == 0) && (tempoCounter==SHIFT_CHORD_TIMING_BY_SAMPLE)) //test - every 2 seconds, at given sample
				{
		...
		if(PROG_enable_rhythm)
		{
			...
			//if (melodyCounter == 1)
			//if (tempoCounter % (TEMPO_BY_SAMPLE/(progressive_rhythm_factor*2)) == 0)
			if(melodyCounter==1)
			{
				LEDs_ORANGE_off();
			}
			...
			if (tempoCounter % (TEMPO_BY_SAMPLE/progressive_rhythm_factor) == 0)
			{
		...
		*/
		...
		if(PROG_enable_LED_indicators_sequencer && melodyCounter==1)
		{
    	...
    	if(PROG_enable_filters)
    	{
			...
			//if (tempoCounter%(2*I2S_AUDIOFREQ/PROGRESS_UPDATE_FILTERS_RATE)==57) //every X ms, at 1.29 ms (57/44100)
    		{
    		...
    		if (TIMING_BY_SAMPLE_EVERY_10_MS == 0) //100Hz periodically, at 0ms
			{
				...
				if(!PROG_audio_input_IR_sensors && ADC_process_sensors()==1)
				{
					...
					if(PROG_enable_S3_control_resonance)
					{
						...
						{
						*/
							if(ADC_last_result[2] > IR_sensors_THRESHOLD_4) //was 400
							{
								fil->iir2[sensors_loop%FILTERS].setResonance(0.999f);
							}
							else if(ADC_last_result[2] > IR_sensors_THRESHOLD_2) //was 200
							{
								fil->iir2[sensors_loop%FILTERS].setResonance(0.996f);
							}
							else if(ADC_last_result[2] > IR_sensors_THRESHOLD_1) //was 100
							{
			...
			if(PROG_enable_S4_control_arpeggiator)
			{
				...
				//equals to: if(sampleCounter%(2*I2S_AUDIOFREQ/12==441))	//12Hz
				//if((melodyCounter % (MELODY_BY_SAMPLE/8)) == 1)
				if(melodyCounter == SHIFT_ARP_TIMING_BY_SAMPLE)
				{
					if(ADC_last_result[3] > IR_sensors_THRESHOLD_4) //was 400
					{
						...
						if(ADC_last_result[3] > IR_sensors_THRESHOLD_6) //was 800
						{
